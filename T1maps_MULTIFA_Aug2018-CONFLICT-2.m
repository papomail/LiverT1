...
position = round(wait(h));
close
xrange2=position(1):position(1)+position(3);
yrange2=position(2):position(2)+position(4);
...
dataT1_ROI=squeeze(reshaped_dataT1(yrange2,xrange2,:,:)).*rsquarefilt_mat(yrange2,xrange2,:,:);%% Mask out voxels with R^2<09 fot the single fit
