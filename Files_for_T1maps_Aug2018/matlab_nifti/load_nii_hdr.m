...
function [hdr, filetype, fileprefix, machine] = load_nii_hdr(fileprefix)
   ...
   myfs=findstr('.nii',fileprefix); %%// Change: Modified to accept filemames with multiple instances of '.nii' string (eg. spirTSE_ti100.nii_FLIRT.nii). PT
   if myfs
       if numel(myfs)>1
           new_ext = 1;
           fileprefix = fileprefix(1:myfs(end)-1);
       else                 %%// Endof Change
           new_ext = 1;
